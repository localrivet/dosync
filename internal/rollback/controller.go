...
func (rc *RollbackControllerImpl) Rollback(service string) error {
	...
	// Restart the service with docker compose (--no-deps to avoid trying to recreate dependencies)
	cmd := execCommand("docker", "compose", "-f", rc.Config.ComposeFilePath, "up", "-d", "--no-deps", service)
